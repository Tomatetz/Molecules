...
(function(informatics, structures, actions) {
	'use strict';
	actions.AddAction = function(sketcher, a, as, bs, GWS) {
	...
	_.innerForward = function() {
        if(!this.sketcher.atomsCount){
            this.sketcher.atomsCount = 1;
        }
		var mol = this.sketcher.getMoleculeByAtom(this.a);
        if(!this.a.atomNumber){
            this.a.atomNumber = this.sketcher.atomsCount;
        }
		if (!mol) {
		...
		if (this.as) {
			for ( var i = 0, ii = this.as.length; i < ii; i++) {
                this.sketcher.atomsCount += 1;
                this.as[i].atomNumber = this.sketcher.atomsCount;
...
(function(actions, atomConn) {
	'use strict';
	actions.GWSAttachAction = function(a, delta) {
		this.a = a;
	};
	var _ = actions.GWSAttachAction.prototype = new actions._Action();
	_.innerForward = function() {
        var bondsQ = 0;
        if(!this.a.AttachQuant){
            this.a.AttachQuant = 0;
        }
        $.each(this.a.bonds, function(i, bond){
            bondsQ += bond.bondOrder!=10?bond.bondOrder:0;
        });
        this.a.maxAttachQuant = atomConn(this.a.label, bondsQ);
        if(this.a.maxAttachQuant>0){
            //if((this.a.AttachQuant+1)<=this.a.maxAttachQuant){
            //    this.a.AttachQuant = this.a.AttachQuant+1;
            if(this.a.GWSAttachOption){
                this.a.GWSAttachOption = false;
                $( "body").trigger( "event", [{
                    action:"Remove Attach Action",
                    atoms:[this.a.atomNumber],
                    type:'delete',
                    event:'attach',
                    order: null
                }] );
            } else {
                this.a.GWSAttachOption = true;
                $( "body").trigger( "event", [{
                    action:"GWS Attach Action",
                    atoms:[this.a.atomNumber],
                    type:'add',
                    event:'attach',
                    order: null
            ...
            this.a.blinkError = true;
        }
	};
	_.innerReverse = function() {
        console.log(this.a);
        $('.log-insert:first-child').remove();
        this.a.GWSAttachOption?this.a.GWSAttachOption=false:this.a.GWSAttachOption=true;
	};

})(ChemDoodle.uis.actions,ChemDoodle.atomConn);
(function(actions, ChemDoodle) {
	'use strict';
	actions.GWSInsertAction = function(a, sketcher,  selectedAtoms) {
		this.a = a;
        this.sketcher = sketcher;
        this.selectedAtoms = selectedAtoms;
	};
	var _ = actions.GWSInsertAction.prototype = new actions._Action();
	_.innerForward = function() {
        ...
        $.each(this.sketcher.molecules, function(i,molecule) {
            $.each(molecule.atoms, function (j, atom) {
                if (atom.GWSInsertOrder) {
                    var concatinated = insertOrders.concat(atom.GWSInsertOrder);
                    insertOrders = unique(concatinated.sort());
                }
            });
        });
        if(this.selectedAtoms.length>0){
            var selectedAtomsNumbers = [];
            var fX, fY, lX, lY;
            $.each(this.selectedAtoms, function(i, atom){
                if(!fX){
                    fX = atom.x;
                    fY = atom.y;
                    lX = atom.x;
                ...
                selectedAtomsNumbers.push(atom.atomNumber);
            });
            if(!this.sketcher.square){
                this.sketcher.square = [];
            }
            $.each(this.sketcher.square, function(i,sq){
                if(fX-10==sq.fx||lX+10==sq.lx){
                    if(lX-sq.fx+20>sq.lx-sq.fx){
                        fX -= 10;
                        lY +=10;
                        lX +=10;
                        fY -=10;
            ...
            $.each(this.selectedAtoms, function(j,atom){
                atom.GWSInsertOption = true;
                atom.GWSInsertOrder = atom.GWSInsertOrder?atom.GWSInsertOrder.concat(insertOrders[insertOrders.length-1]+1):[0,insertOrders[insertOrders.length-1]+1];
            });
            this.sketcher.square.push( {
                fx: fX-10,
                fy: fY-10,
                ly: lY+10,
                lx: lX+10,
                atoms: selectedAtomsNumbers,
                name: 'G'+(insertOrders[insertOrders.length-1]+1)
            });
            $( "body").trigger( "event", [{
                action:"GWS Insert Action",
                atoms:selectedAtomsNumbers,
                type:'add',
                event:'insert',
                order: insertOrders[insertOrders.length-1]+1
            }] );
            ...
            $.each(this.sketcher.molecules, function(i,molecule){
                $.each(molecule.rings, function(j,ring){
                    ...
                    if(inRing.length<ring.atoms.length-1&&inRing.length!=0){
                        $.each(molecule.atoms, function(i, atom){
                                atom.isLassoed = false;
                        });
                    }
                });
            ...
            this.a.GWSInsertOption = this.a.GWSInsertOption?false:true;
            if(this.a.GWSInsertOption){
                $( "body").trigger( "event", [{
                    action:"GWS Insert Action",
                    atoms:[this.a.atomNumber],
                    type:'add',
                ...
                this.a.GWSInsertOrder = [0,insertOrders[insertOrders.length-1]+1];
                insertOrders.push(insertOrders[insertOrders.length-1]+1);
            } else {
                $( "body").trigger( "event", [{
                    action:"GWS Remove Insert Action",
                    atoms:[this.a.atomNumber],
                    type:'delete',
                    event:'insert'
                }] );
                delete this.a.GWSInsertOrder;
            }
        }
	};
	_.innerReverse = function() {
        ...
        if(this.selectedAtoms.length>0){
            var inserts = this.selectedAtoms[0].GWSInsertOrder;
            $.each(this.selectedAtoms, function(i, atom){
                inserts = inserts.diff(atom.GWSInsertOrder);
            });

            $.each(this.selectedAtoms, function(i, atom){
                atom.GWSInsertOrder.splice(atom.GWSInsertOrder.indexOf(inserts[0]),1);
                if(atom.GWSInsertOrder.length==1){
                    atom.GWSInsertOption = false;
                    delete atom.GWSInsertOrder;
                }
            });
        } else {
            this.a.GWSInsertOption = false;
        }
        $('.log-insert:first-child').remove();
	};

})(ChemDoodle.uis.actions, ChemDoodle);
(function(actions) {
	'use strict';
...
(function(structures, actions) {
	...
	actions.ClearAction = function(sketcher) {
		...
		this.beforeAtomsCount = this.sketcher.atomsCount;
		this.sketcher.clear();
	...
	_.innerReverse = function() {
        ...
        this.sketcher.atomsCount = this.beforeAtomsCount;
	};
...
(function(math, monitor, actions, states, structures, SYMBOLS, m) {
	...
	_.mousewheel = function(e, delta) {
        ...
        var rotateBuffer = this.sketcher.specs.scale;
		this.sketcher.repaint();
	};
	_.drag = function(e) {
		if (this.innerdrag) {
		...
		if (!this.sketcher.hovering && this !== this.sketcher.stateManager.STATE_LASSO && this !== this.sketcher.stateManager.STATE_SHAPE && this !== this.sketcher.stateManager.STATE_PUSHER) {
			if (monitor.SHIFT) {
                ...
                $.each(this.sketcher.square, function(i, square){
                    square.fx += dif.x;
                    square.fy += dif.y;
                    square.lx += dif.x;
                    square.ly += dif.y;
...
(function(actions, states, structures) {
	'use strict';
	states.GWSAttachState = function(sketcher) {
		this.setup(sketcher);
	};
	var _ = states.GWSAttachState.prototype = new states._State();
	_.innermouseup = function(e) {
		if (this.sketcher.hovering) {
            this.sketcher.hovering.isSelected = false;
			this.sketcher.historyManager.pushUndo(new actions.GWSAttachAction(this.sketcher.hovering));
		}
	};
...
(function(actions, states, structures) {
	'use strict';
	states.GWSExportJson = function(sketcher) {
		this.setup(sketcher);
	};
	var _ = states.GWSExportJson.prototype = new states._State();
	_.innermouseup = function(e) {
        $('.pictureInput').css('border-bottom','1px solid red')
	};
    _.innermousedown = function(e) {
    };
	_.innermousemove = function(e) {
...
(function(actions, states, structures) {
	'use strict';
	states.GWSInsertState = function(sketcher) {
		this.setup(sketcher);
	};
	var _ = states.GWSInsertState.prototype = new states._State();
	_.innermouseup = function(e) {
        this.newMolAllowed = true;
		if (this.sketcher.hovering) {
...
(function(monitor, actions, states, structures, m, atomConn) {
	'use strict';
	states.GWSClosingState = function(sketcher) {
		this.setup(sketcher);
	};
	var _ = states.GWSClosingState.prototype = new states._State();
    _.innerdrag = function(e) {
        this.newMolAllowed = false;
        this.removeStartAtom();
        if (this.sketcher.hovering instanceof structures.Atom || this.sketcher.hovering.x) {
    ...
    _.innermouseup = function(e) {
        $.each(this.sketcher.molecules, function(i, molecule){
            $.each(molecule.atoms, function(j,atom){
                atom.order = j.toString()+i;
            });
            $.each(molecule.rings, function(k, ring){
               return ring.bonds.every(function(j,bond){
                   return bond.isGWS == false;
               })
            });
        });
        if (this.sketcher.tempAtom && this.sketcher.hovering) {
            ...
            if (this.sketcher.tempAtom.isOverlap) {
                ...
                var a1 = this.sketcher.tempAtom,
                    a2 = this.sketcher.hovering;

                var bondsQ1 = 0,
                    bondsQ2 = 0;
                $.each(a1.bonds, function(i, bond){
                    bondsQ1 += bond.bondOrder!=10?bond.bondOrder:0;
                });
                $.each(a2.bonds, function(i, bond){
                    bondsQ2 += bond.bondOrder!=10?bond.bondOrder:0;
                });
                var bond = this.sketcher.getBond(this.sketcher.hovering, this.sketcher.tempAtom),
                    atomConn1 = atomConn(a1.label, bondsQ1),
                    atomConn2 = atomConn(a2.label, bondsQ2);
                if (!bond) {
                    if(atomConn1>0&&atomConn2>0){
                        ...
                        $.each(this.sketcher.molecules, function(i,molecule){
                            $.each(molecule.rings, function(j,ring){
                                var inRA1 = false,
                                    inRA2 = false;
                                $.each(ring.atoms, function(k,atom){
                                    if(atom.order == a1.order){
                                        inRA1 = true;
                                    } else if (atom.order == a2.order){
                                        inRA2 = true;
                                    }
                                    if(inRA1 && inRA2){
                                        pass = false
                        ...
                        if(pass){
                            $( "body").trigger( "event", [{
                                action:"GWS Closing Action",
                                atoms:[a2.atomNumber, a1.atomNumber],
                                type:'add',
                                event:'closing',
                                order: null
                            }] );
...
})(ChemDoodle.monitor, ChemDoodle.uis.actions, ChemDoodle.uis.states, ChemDoodle.structures, Math, ChemDoodle.atomConn);
//
//  Copyright 2009 iChemLabs, LLC.  All rights reserved.
//
...
(function(states) {
	...
	states.StateManager = function(sketcher) {
		...
		this.STATE_GWSATTACH = new states.GWSAttachState(sketcher);
		this.STATE_GWSINSERT = new states.GWSInsertState(sketcher);
		this.STATE_EXPORTJSON = new states.GWSExportJson(sketcher);
		this.STATE_GWSCLOSING = new states.GWSClosingState(sketcher);
...
ChemDoodle.uis.gui.imageDepot = (function() {
	...
	d.BOND_ANY = '';
	...
	d.CLEAR = '';
	...
	d.DECREASE_CHARGE = '';
	...
	d.ERASE = '';
	...
	d.INCREASE_CHARGE = '';
	...
	d.LASSO = '';
	d.LASSO_SHAPES = '';
	d.MARQUEE = '';
	...
	d.OPEN = '';
	...
	d.REDO = '';
	...
	d.UNDO = '';
	d.ZOOM_IN = '';
	d.ZOOM_OUT = '';

    d.INSERT = '';
    d.ATTACH = '';
    d.CLOSING = '';
    d.EXPORT = '';
...
(function(c, actions, gui, desktop, q) {
	...
	gui.DialogManager = function(sketcher) {
		...
		// }
		//sb.push(' or <input class="input_molfile" type="file">');
		...
		this.loadDialog.buttons = {
			'Load' : function() {
				...
				if (sketcher.oneMolecule && newContent && newContent.molecules.length > 0 && newContent.molecules[0].atoms.length > 0) {
                    ...
                    console.log(newContent.molecules);
...
(function(desktop, imageDepot, q, document) {
	...
	_.getHiddenSource = function() {
		var sb = [];
		sb.push('<div style="display:none;position:absolute;z-index:10;border:1px #C1C1C1 solid;background:#E2E2E2;padding:5px;border-bottom-left-radius:2px;-moz-border-radius-bottomleft:2px;border-bottom-right-radius:2px;-moz-border-radius-bottomright:2px;" id="');
		sb.push(this.id);
		sb.push('_hidden">');
...
//  Copyright 2009 iChemLabs, LLC.  All rights reserved.
//
(function(c, iChemLabs, io, structures, actions, gui, imageDepot, desktop, tools, states, q, document, protons) {
	'use strict';
	gui.ToolbarManager = function(sketcher) {
		this.sketcher = sketcher;
		// open
		this.buttonOpen = new desktop.Button(sketcher.id + '_button_open', imageDepot.OPEN, 'Load molecule', function() {
		...
		// erase
		this.buttonExportJSON = new desktop.Button(sketcher.id + '_button_exportJSON', imageDepot.EXPORT, 'Form JSON', function() {
            //if(!$('.pictureInput').val()){
            //    $('.pictureInput').css('border-bottom','1px solid red');
            //    $('.img-wrapper').css('color', 'red');
            //} else {
            ChemDoodle.molecules = sketcher.molecules;
                var GWS={
                    atoms: {
                        "atomic_number": [ ],
                        "attach": [],
                ...
                $.each(sketcher.molecules, function(i,molecule){
                    //GWS.rings = GWS.rings.concat(molecule.rings);
                    $.each(molecule.atoms, function(j,atom){
                        atom.orderNum = j;
                        GWS.atoms.atomic_number.push(protons[atom.label]);
                        if(atom.GWSInsertOption){
                            $.each(atom.GWSInsertOrder, function(key, value){
                                if(GWS.atoms.insert[value]){
                                    var temp = GWS.atoms.insert[value];
                                    temp.push(j);
                                    GWS.atoms.insert[value] = temp;
                                } else {
                                    GWS.atoms.insert[value] = [j];
                                }
                            });
                        }
                        if(atom.GWSAttachOption){
                            for(var k=0; k<atom.AttachQuant; k++){
                                GWS.atoms.attach.push(j);
                            }
                        }
                    });
                    $.each(molecule.bonds, function(j,bond){
                        var curBond =[];
                        var atoms = [];
                        $.each(Object.keys(bond), function(k, atom){
                            if(atom[0]=='a'){
                                atoms.push(bond[atom]);
                                curBond.push(bond[atom].orderNum);
                            }
                        });
                        curBond.push(bond.bondOrder);
                        GWS.bonds.push(curBond);
                    });
                });
                if(GWS.atoms.insert[0]){
                    delete GWS.atoms.insert[0];
                }
                //console.log(sketcher);
                GWS.name = $('.pictureInput').val();
                //console.log(GWS);
            //}
			//sketcher.stateManager.setState(sketcher.stateManager.STATE_EXPORTJSON);
		});
		//this.buttonExportJSON.toggle = false;
		...
		this.makeGWSSet(this);
	};
	var _ = gui.ToolbarManager.prototype;

	_.write = function() {
		var sb = ['<div style="font-size:10px;text-align:center;">'];
        ...
        sb.push(this.gwsSet.getSource(bg));
		sb.push('</div>');
    ...
    _.makeGWSSet = function(self) {
        this.gwsSet = new desktop.ButtonSet(self.sketcher.id + '_buttons_gws');
        this.buttonInsert = new desktop.Button(self.sketcher.id + '_button_insert', imageDepot.INSERT, 'Insert', function() {
            self.sketcher.stateManager.setState(self.sketcher.stateManager.STATE_GWSINSERT);
        });
        this.gwsSet.buttons.push(this.buttonInsert);
        this.buttonAttach = new desktop.Button(self.sketcher.id + '_button_attach', imageDepot.ATTACH, 'Attach', function() {
            self.sketcher.stateManager.setState(self.sketcher.stateManager.STATE_GWSATTACH);
        });
        this.gwsSet.buttons.push(this.buttonAttach);
        this.buttonClosing = new desktop.Button(self.sketcher.id + '_button_Closing', imageDepot.CLOSING, 'Closing', function() {
            self.sketcher.stateManager.setState(self.sketcher.stateManager.STATE_GWSCLOSING);
            self.sketcher.stateManager.STATE_GWSCLOSING.bondOrder = 10;
        });
        this.gwsSet.buttons.push(this.buttonClosing);
    };
...
})(ChemDoodle, ChemDoodle.iChemLabs, ChemDoodle.io, ChemDoodle.structures, ChemDoodle.uis.actions, ChemDoodle.uis.gui, ChemDoodle.uis.gui.imageDepot, ChemDoodle.uis.gui.desktop, ChemDoodle.uis.tools, ChemDoodle.uis.states, ChemDoodle.lib.jQuery, document, ChemDoodle.protons);
//
//  Copyright 2009 iChemLabs, LLC.  All rights reserved.
//
...
(function(c, extensions, featureDetection, sketcherPack, structures, tools, q, m, window) {
	...
	_.drawSketcherDecorations = function(ctx) {
        ...
        if(this.square){
            $.each(this.square, function(i,square){
                ctx.save();
                //ctx.setLineDash([2, 1]);
                ctx.rect(square.fx,square.fy,square.lx-square.fx,square.ly-square.fy);
                ctx.stroke();
                ctx.setLineDash([]);
                ctx.font = "10px Arial";
	...
	_.drawChildExtras = function(ctx) {
		...
		if (!this.paidToHideTrademark) {
			...
			ctx.fillText('', this.width - width - 13, this.height - 4);
...
(function(c, iChemLabs, io, structures, actions, gui, imageDepot, desktop, tools, states, q, document) {
	...
	_.write = function() {
		...
		sb.push(this.buttonClear.getSource());
		sb.push(this.buttonInsert.getSource());
		if (this.editor.useServices) {
			sb.push(this.buttonClean.getSource());
		}
		sb.push('</div>');
